...
from ultralytics.data.rgb_ir import bgr_hwc_to_rgb_chw, rgb_view
...
class BasePredictor:
    ...
    def preprocess(self, im):
        ...
        if not_tensor:
            ...
            im = np.stack([bgr_hwc_to_rgb_chw(x) for x in transformed])
    ...
    def setup_source(self, source):
        ...
        self.dataset = load_inference_source(source=source,
                                             ...
                                             ir_source=getattr(self.args, 'ir_source', None),
                                             data=getattr(self.args, 'data', None),
                                             ch=getattr(self.args, 'ch', 3),
                                             resize_ir=getattr(self.args, 'pair_resize', False))
        self.bs = self.dataset.bs
        ...
        with self._lock:  # for thread-safe inference
            ...
            for batch in self.dataset:
                ...
                for i in range(n):
                    ...
                    p = Path(path[i])
                    im0 = None if self.source_type.tensor else rgb_view(im0s[i]).copy()
                    if self.args.verbose or self.args.save or self.args.save_txt or self.args.show:
                        s += self.write_results(i, self.results, (p, im[:, :3], im0))
                    if self.args.save or self.args.save_txt:
