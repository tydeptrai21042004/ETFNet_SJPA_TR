...
class Model(nn.Module):
    ...
    def train(self, trainer=None, **kwargs):
        ...
        if os.getenv("ULTRALYTICS_ALLOW_NETWORK", "0").strip().lower() in {"1", "true", "yes"}:
            checks.check_pip_update_available()

        overrides = yaml_load(checks.check_yaml(kwargs['cfg'])) if kwargs.get('cfg') else self.overrides
        is_resume = bool(kwargs.get('resume'))
        # A resume checkpoint owns its original training configuration.  In particular, do not inject the
        # framework's task-default dataset here: BaseTrainer.check_resume() reads the saved train_args and only
        # replaces data when the caller explicitly supplied a relocated YAML path.
        custom = {} if is_resume else {'data': DEFAULT_CFG_DICT['data'] or TASK2DATA[self.task]}
