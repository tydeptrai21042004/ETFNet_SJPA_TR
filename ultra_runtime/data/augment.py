...
from ultralytics.data.rgb_ir import bgr_hwc_to_rgb_chw
...
class RandomHSV:
    ...
    def __call__(self, labels):
        """Apply color jitter without mixing or reordering RGB and IR channels."""
        image = labels['img']
        if not (self.hgain or self.sgain or self.vgain) or image.ndim != 3:
            return labels
        if image.shape[2] % 3:
            raise ValueError(f'RandomHSV expects channels in 3-channel modality groups, got {image.shape}')

        gains = np.random.uniform(-1, 1, 3) * [self.hgain, self.sgain, self.vgain] + 1
        dtype = image.dtype
        lut_x = np.arange(0, 256, dtype=gains.dtype)

        def transform(modality, rgb_modality=True):
            hsv = cv2.cvtColor(modality, cv2.COLOR_BGR2HSV)
            hue, sat, val = cv2.split(hsv)
            hue_gain = gains[0] if rgb_modality else 1.0
            sat_gain = gains[1] if rgb_modality else 1.0
            lut_hue = ((lut_x * hue_gain) % 180).astype(dtype)
            lut_sat = np.clip(lut_x * sat_gain, 0, 255).astype(dtype)
            lut_val = np.clip(lut_x * gains[2], 0, 255).astype(dtype)
            hsv = cv2.merge((cv2.LUT(hue, lut_hue), cv2.LUT(sat, lut_sat), cv2.LUT(val, lut_val)))
            return cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)

        # The first modality is visible RGB. Thermal/IR receives only the value
        # perturbation so artificial hue and saturation are not introduced.
        image[..., :3] = transform(image[..., :3], rgb_modality=True)
        for start_channel in range(3, image.shape[2], 3):
            image[..., start_channel:start_channel + 3] = transform(
                image[..., start_channel:start_channel + 3], rgb_modality=False)
        labels['img'] = image
        return labels
...
class Format:
    ...
    def _format_img(self, img):
        """Convert every modality independently from BGR/HWC to RGB/CHW."""
        if len(img.shape) < 3:
            img = np.expand_dims(img, -1)
        return torch.from_numpy(bgr_hwc_to_rgb_chw(img))
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    transforms = [pre_transform, MixUp(dataset, pre_transform=pre_transform, p=hyp.mixup)]
    # The stock Albumentations pipeline assumes a 1- or 3-channel image and can
    # corrupt or reject paired six-channel arrays. Geometric transforms above
    # already operate jointly on both modalities, so disable this optional stage
    # for RGB--IR data rather than applying inconsistent per-modality geometry.
    if int(getattr(hyp, 'ch', 3)) <= 3:
