...
def check_det_dataset(dataset, autodownload=True):
    ...
    path_keys = ('train', 'val', 'test', 'train_ir', 'val_ir', 'test_ir',
                 'train_labels', 'val_labels', 'test_labels', 'rgb_root', 'ir_root', 'labels_root')
    for k in path_keys:
        if data.get(k):
    ...
    for split in ('train', 'val', 'test'):
        rgb_value, ir_value = data.get(split), data.get(f'{split}_ir')
        if rgb_value and ir_value:
            ir_paths = [Path(x).resolve() for x in (ir_value if isinstance(ir_value, list) else [ir_value])]
            if not all(x.exists() for x in ir_paths):
                raise FileNotFoundError(f"Dataset '{dataset}' missing {split}_ir path: "
                                        f"{[x for x in ir_paths if not x.exists()][0]}")
