...
def torch_safe_load(weight):
    ...
    try:
        with temporary_modules({
            ...
            return torch_load(file, map_location='cpu'), file  # load
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus, C2fCIB, SCDown, PSA, C3k2, CAFEM, C2PSA,
                 BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3, MF, A2C2f):
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)

            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, C3k2, CAFEM, C2PSA, C2fCIB, A2C2f):
            ...
            if not isinstance(f, list) or len(f) < 2:
                raise ValueError('Add requires at least two input feature indices')
            input_channels = [ch[x] for x in f]
            if len(set(input_channels)) != 1:
                raise ValueError(f'Add inputs must have identical channels, got {input_channels}')
            c2 = input_channels[0]
        elif m is Add2:
            # print("ch[f]", f, ch[f[0]])
            c2 = ch[f[0]]
        ...
        #    args = [c2]
            ...
            c2 = ch[f[0]]
            args = [c2, *args[1:]]
        elif m in (GOCI, SJPA, RTPF, DCSPF, MPCRF, CCPRF):
            c1 = ch[f[0]]
            c2 = 2 * c1
            args = [c1, *args]
