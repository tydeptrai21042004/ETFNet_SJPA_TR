...
def check_font(font='Arial.ttf'):
    """Find a local font without introducing an implicit network dependency.

    Set ``ULTRALYTICS_ALLOW_NETWORK=1`` to retain the optional upstream font
    download behavior. Plotting otherwise falls back to PIL's bundled font.
    """
    name = Path(font).name
    file = USER_CONFIG_DIR / name
    if file.exists():
        return file
    requested = Path(name).stem.lower()
    system_fonts = font_manager.findSystemFonts()
    matches = [s for s in system_fonts if requested in Path(s).stem.lower()]
    if matches:
        return matches[0]
    # A deterministic local fallback is preferred for offline research runs.
    fallbacks = [s for s in system_fonts if 'dejavusans' in Path(s).stem.lower()]
    if fallbacks:
        return sorted(fallbacks)[0]
    if os.getenv('ULTRALYTICS_ALLOW_NETWORK', '0') == '1':
