...
import ultralytics as _ultralytics_package
...
# Resolve resources from the logical ultralytics package root in both the split source
# tree and a conventional installed wheel.
ROOT = Path(_ultralytics_package.__file__).resolve().parent
...
def is_online() -> bool:
    """Return network availability only when network access was explicitly enabled.

    Research runs are offline by default to prevent hidden update checks, downloads,
    or connectivity-dependent startup delays. Set ``ULTRALYTICS_ALLOW_NETWORK=1``
    before process startup to opt into upstream online behavior.
    """
    if os.getenv("ULTRALYTICS_ALLOW_NETWORK", "0").strip().lower() not in {"1", "true", "yes"}:
        return False
