...
def init_seeds(seed=0, deterministic=False):
    """Initialize Python, NumPy, and PyTorch RNGs with deterministic backend controls."""
    seed = int(seed)
    ...
    if deterministic:
        # cuBLAS requires this before the first deterministic GEMM. Setting it
        # here is still effective for normal training entry points, which call
        # init_seeds before the first model forward pass.
        ...
        if self.enabled:
            ...
            for k, v in self.ema.state_dict().items():
                ...
                if v.dtype.is_floating_point:  # true for FP16 and FP32
                    v.mul_(d).add_(source, alpha=1 - d)
                else:
                    # Counters and other integral buffers must follow the raw
                    # model. Leaving them at construction values breaks modules
                    # whose cache/update logic depends on those buffers.
                    v.copy_(source)
            # Invalidate derived evaluation caches after in-place EMA updates.
